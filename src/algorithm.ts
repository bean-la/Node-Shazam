...
export class SignatureGenerator {
    ...
    doPeakRecognition() {
        ...
        for (let binPosition = 10; binPosition < 1015; binPosition++) {
            ...
            const fftMinus46Value = fftMinus46[binPosition];
            const fftMinus49PrevValue = fftMinus49[binPosition - 1];

            if (fftMinus46Value !== undefined && fftMinus49PrevValue !== undefined &&
                (fftMinus46Value >= 1 / 64) && (fftMinus46Value >= fftMinus49PrevValue)) {
                ...
                for (const neighborOffset of [...range(-10, -3, 3), -3, 1, ...range(2, 9, 3)]) {
                    ...
                    if (candidate === undefined || isNaN(candidate)) continue;
                    maxNeighborInFftMinus49 = Math.max(candidate, maxNeighborInFftMinus49);
                }
                if (fftMinus46Value > maxNeighborInFftMinus49) {
                    let maxNeighborInOtherAdjacentFFTs = maxNeighborInFftMinus49;
                    for (const otherOffset of [-53, -45, ...range(165, 201, 7), ...range(214, 250, 7)]) {
                        const otherFft = this.spreadFFTsOutput.list[pyMod(this.spreadFFTsOutput.position + otherOffset, this.spreadFFTsOutput.bufferSize)];
                        if (!otherFft) continue;
                        const candidate = otherFft[binPosition - 1];
                        if (candidate === undefined || isNaN(candidate)) continue;
                    ...
                    if (fftMinus46Value > maxNeighborInOtherAdjacentFFTs) {
                        // This is a peak. Store the peak

                        const fftNumber = this.spreadFFTsOutput.written - 46;

                        const peakMagnitude = Math.log(Math.max(1 / 64, fftMinus46Value)) * 1477.3 + 6144,
                            peakMagnitudeBefore = Math.log(Math.max(1 / 64, fftMinus46[binPosition - 1] ?? 1 / 64)) * 1477.3 + 6144,
                            peakMagnitudeAfter = Math.log(Math.max(1 / 64, fftMinus46[binPosition + 1] ?? 1 / 64)) * 1477.3 + 6144;

                        const peakVariation1 = peakMagnitude * 2 - peakMagnitudeBefore - peakMagnitudeAfter,
                            peakVariation2 = (peakVariation1 > 0) ? (peakMagnitudeAfter - peakMagnitudeBefore) * 32 / peakVariation1 : 0;
